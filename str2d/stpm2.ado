*! version 1.3.5 02Feb2011

/*
History
PR 13mar2011: temporary from() option for initial values
PL 07mar2010: added an option that will try using the lininit option if convergence using default fails
...
PL 21sep2009: added rcsbaselineoff option
...
program Estimate, eclass byable(recall)
	...
	[, DF(string) TVC(varlist fv) DFTvc(string) KNOTS(numlist ascending) KNOTSTvc(string) ///
		...
		ALL RMAT REVerse CURE FAILCONVLININIT /* !!PR temp */ from(string) ] ///
	[                               ///
		...
		foreach tvcvar in  `tvc' {
			if "`tvcknots_`tvcvar'_user'" == "" {
				if `tvc_`tvcvar'_df' == 1 {
					...
					qui _pctile `lnt' if `touse' & _d==1 `wt', p(12.5 25 37.5 50 62.5 75 87.5) 
					local tvcknots_`tvcvar' `lowerknot' `r(r1)' `r(r2)' `r(r3)' `r(r4)' `r(r5)' `r(r6)' `r(r7)' `upperknot'
				}
				else if `tvc_`tvcvar'_df'==9 {
					qui _pctile `lnt' if `touse' & _d==1 `wt', p(11.1 22.2 33.3 44.4 55.6 66.7 77.8 88.9) 
					local tvcknots_`tvcvar' `lowerknot' `r(r1)' `r(r2)' `r(r3)' `r(r4)' `r(r5)' `r(r6)' `r(r7)' `r(r8)' `upperknot'
				}
				else if `tvc_`tvcvar'_df'==10 {
					qui _pctile `lnt' if `touse' & _d==1 `wt', p(10 20 30 40 50 60 70 80 90) 
					local tvcknots_`tvcvar' `lowerknot' `r(r1)' `r(r2)' `r(r3)' `r(r4)' `r(r5)' `r(r6)' `r(r7)' `r(r8)' `r(r9)' `upperknot'
				}
				else {
...
/* Generate splines for delayed entry */
	if `del_entry' == 1 {
		...
		if "`df'" == "1" & "`rcsbaseoff'" == "" {
			qui rcsgen `lnt0' if `touse2' & _t0>0, gen(_s0_rcs1) `reverse'
		}
...
/* If further constraints are listed stpm2 then remove this from mlopts and add to conslist */
	...
	/* !! PR temporary addition for initialisation from `from' */
	if "`from'" == "" {
		if "`lininit'" == "" {
