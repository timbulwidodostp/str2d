*! version 1.3.5 22Mar2011

/*
History
PR 22mar2011: minor bug in tmin() option, fixed.
...
program stpm2_pred
	...
	syntax newvarname [if] [in], [Survival Hazard XB XBNOBaseline DXB DZDY HRNumerator(string) HRDenominator(string) MEANSurv ///
									...
									RMst TMAx(string) TMIn(string) n(int 1000) ABc hr0(string) first(int 1)]
...
if "`tmax'" != "" {
		if "`rmst'`abc'" == "" {
			display as error "tmax() valid only with rmst or abc"
			exit 198
		}
		confirm number `tmax'
		if `tmax' <= 0 {
	...
	if "`tmin'" != "" {
		if "`rmst'`abc'" == "" {
			display as error "tmin() valid only with rmst or abc"
			exit 198
		}
		confirm number `tmin'
		if `tmin' < 0 {
	...
	if wordcount(`"`survival' `hazard' `meansurv' `hratiotmp' `sdifftmp' `hdifftmp' `centile' `xb' `xbnobaseline' `dxb' `dzdy' `martingale' `deviance' `cumhazard' `cumodds' `normal' `density' `tvc' `cure'`rmst'"') > 1 {
		display as error "You have specified more than one option for predict"
		exit 198
	}
	if wordcount(`"`survival' `hazard' `meansurv' `hrnumerator' `sdiff1'  `hdifftmp' `centile' `xb' `xbnobaseline' `dxb' `dzdy' `martingale' `deviance' `cumhazard' `cumodds' `normal' `density' `tvc' `cure'`rmst'`abc'"') == 0 {
	...
	if "`ci'" != "" & ///
		wordcount(`"`survival' `hazard' `hrnumerator' `sdiff1' `hdiff1' `centile' `xb' `dxb' `xbnobaseline' `tvc' `meansurv' `cure' `rmst' `abc'"') == 0 {
...
/* call stpm2_mttf for SE and CI if rmst option specified */
	if "`rmst'" != "" {
		if "`at'" != "" local At at(`at')
		cap confirm var `newvarname'
		if c(rc) != 0 {
			qui _rmst if `touse', tmin(`tmin') tmax(`tmax') generate(`newvarname') `At' `zeros' n(`n')
			if "`ci'`stdp'" == "" {
				exit
			}
		}
		...
		qui predictnl `newvarname' = predict(rmst tmin(`tmin') tmax(`tmax') `At' `zeros' n(`n')) `predictnl_opts' 
		exit
	}
			...
			while "`1'"!="" {
				...
				if _rc {
					if "`2'" == "." {
						...
						cap confirm num `2'
						if _rc {
							di in red "invalid hrdenominator(... `1' `2' ...)"
							exit 198
						}
					}
				}
	...
	/* Predicted survival time among uncured for a given centile */
				...
				if "`ci'" != "" {
					...
					foreach tvcvar in `e(tvc)' {
						...
						if `e(df_`tvcvar')' == 1 {
							qui rcsgen `ln_nr_time' if `touse',  gen(_rcs_`tvcvar') dgen(_d_rcs_`tvcvar') `e(reverse)' `rmatrix'
						}
						else if `e(df_`tvcvar')' != 1 {
							qui rcsgen `ln_nr_time' if `touse', knots(`e(ln_tvcknots_`tvcvar')') gen(_rcs_`tvcvar') dgen(_d_rcs_`tvcvar') `e(reverse)' `rmatrix'
						}
...
/* loop */
		...
		if "`ci'" != "" {
			...
			if "`e(rcsbaseoff)'" == "" {
				...
				qui rcsgen `ln_nr_time'  if `touse', knots(`e(ln_bhknots)') gen(_rcs) dgen(_d_rcs) `e(reverse)' `rmatrix'
			...
			foreach tvcvar in `e(tvc)' {
				if "`e(orthog)'" != "" {
					tempname rmatrix_`tvcvar'
					matrix `rmatrix_`tvcvar'' = e(R_`tvcvar')
					local rmatrix rmatrix(`rmatrix_`tvcvar'')
				}
				if `e(df_`tvcvar')' == 1 {
					qui rcsgen `ln_nr_time' if `touse',   gen(_rcs_`tvcvar') dgen(_d_rcs_`tvcvar') `e(reverse)' `rmatrix'
				}
...
program define Parse_list, rclass
	...
	while "`1'"!="" {
		...
		if _rc {
			...
			if _rc {
				if "`2'" != "." {
					di in red "invalid `listname'(... `1' `2' ...)"
					exit 198
				}
			}
		}
...
/*
	Mean or restricted mean time to failure for an stpm2 model, based on covariate
	patterns or an `at()' specification. Expects stpm2 model to be fit beforehand,
	so only predicts (except for bootstrap, which refits original model).

	Similar to its predecessor stpm2_mttf, but omitting the gense() and reps() options.
*/

version 11.1
syntax [if] [in], [ at(string) GENerate(string) N(int 1000) TMAx(real 0) TMIn(real 0) ZEROs ]
local varlist `e(varlist)'
local survival survival
if !missing("`at'") {
	// Check list of values is comprehensive. Out of sample prediction allowed.
	CheckAt "`at'" "`varlist'"
	if !missing("`s(omitted)'") {
		if !missing("`zeros'") {
			di as err "cannot mix zeros with covariates `s(omitted)' to average over"
			exit 198
		}
		di as txt "note: averaging predicted survival functions over `s(omitted)'"
		local omitted `s(omitted)'
		local survival meansurv
		local ifesample if e(sample)
	}
}
...
quietly {
	...
	gen `integral' = .
	if missing("`at'") {
		// identify covariate patterns and compute mean survival at them
		tempvar covpat
		covpat `varlist' if `touse', generate(`covpat')
		sum `covpat', meanonly
		local ncovpat = r(max)
	...
	forvalues i = 1 / `ncovpat' {
		if missing("`at'") {
			// determine covariate values for this covariate pattern
			local At
			tokenize `varlist'
			while "`1'" != "" {
		...
		// If have covars `omitted' from at(),
		// use meansurv to average survival curves over them.
/*
	!! Note need for "if e(sample)" with -meansurv- option
	(a) when averaging survival over covariates other than those in at(), and
	(b) when predicting to a timevar that could contain out of sample observations.
	If don't do this, get missing prediction of `s' when `n'>`n_orig'.
	This is an inconsistency in the way -predict, survival- and
	-predict, meansurv- behave.
*/
		if ("`survival'" == "meansurv") {
			predict `s' if e(sample), timevar(`t') at(`At') meansurv `zeros'
		}
		else predict `s', timevar(`t') at(`At') survival `zeros'
		replace `s' = 1 if `t' == 0
		integ `s' `t'
		replace `integral' = r(integral) if `covpat' == `i'
		drop `s'
	...
	if !missing("`generate'") {
		...
		if missing("`cmax'") lab var `generate' "restricted mean at time `tmax'"
		else lab var `generate' "mean time"
	}
